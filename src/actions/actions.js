...
export function getFavorites() {
  ...
  return (dispatch) => {
    axios.get(`http://localhost:5000/savedMovies/${JSON.parse(localStorage.getItem("user")).id}`)
      .then((res) => {
        let obj = {}
...
export function addFavorite(movie) {
  ...
  return (dispatch) => {
    axios
      .post(`http://localhost:5000/savedMovies`, movie)
      .then((res) => {
        let obj = {}
        console.log("res.data",res.data)
        res.data.movies.map(item => obj[item.movie_id] = true)
...
export function deleteFavorite(movie_id) {
  ...
  return (dispatch) => {
    axios
      .delete(
        `http://localhost:5000/savedMovies/${movie_id}`)
      .then((res) => {
        let obj = {}
        console.log("res.data",res.data)
        res.data.movies.map(item => obj[item.movie_id] = true)
...
export function deleteRecommedations(movie_id) {
  return (dispatch) => {
    axios
      .delete(
        `http://localhost:5000/${movie_id}/${Number(
...
export function editUser(user) {
  ...
  return (dispatch) => {
    axios
      .put(`http://localhost:5000/users/${localStorage.getItem("id")}`, user)
      .then((res) => {
