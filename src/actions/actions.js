...
export function getFavorites() {
  ...
  return (dispatch) => {
    axios.get(`https://movie-app-be-1.onrender.com/savedMovies/${localStorage.getItem("id")}`)
      .then((res) => {
        let obj = {}
...
export function addFavorite(movie) {
  ...
  return (dispatch) => {
    axios
      .post(`https://movie-app-be-1.onrender.com/savedMovies`, movie)
      .then((res) => {
        let obj = {}
        console.log("res.data",res.data)
        res.data.movies.map(item => obj[item.movie_id] = true)
...
export function deleteFavorite(movie_id) {
  ...
  return (dispatch) => {
    axios
      .delete(
        `https://movie-app-be-1.onrender.com/savedMovies/${movie_id}`)
      .then((res) => {
        let obj = {}
        console.log("res.data",res.data)
        res.data.movies.map(item => obj[item.movie_id] = true)
...
export function recommedations(movie, recommended_movie) {
  ...
  axios
    .post(`https://movie-app-be-1.onrender.com/recommendedMovies`, new_movie)
    .then((res) => {
...
export function deleteRecommedations(movie_id) {
  return (dispatch) => {
    axios
      .delete(
        `https://movie-app-be-1.onrender.com/${movie_id}/${Number(
...
export function getUser() {
  ...
  return (dispatch) => {
    axios
      .get(
        `https://movie-app-be-1.onrender.com/users/${localStorage.getItem("id")}`
      )
      .then((res) => {
        console.log(res.data)
...
export function editUser(user) {
  ...
  return (dispatch) => {
    axios
      .put(`https://movie-app-be-1.onrender.com/users/${localStorage.getItem("id")}`, user)
      .then((res) => {
